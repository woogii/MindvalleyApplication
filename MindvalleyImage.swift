...
open class MindvalleyImage : NSObject {
  ...
  func taskForGetData(urlString: String, parameters: [String : AnyObject]?=nil, size:Int?=nil, completionHandler: @escaping CompletionHandler)->URLSessionDataTask? {
    ...
    let task = URLSession.shared.dataTask(with: request) { (data,response, downloadError) in
      ...
      if let error = downloadError {
        let customizedError = MindvalleyImage.errorForData(data: data, response:response, error:error)
        completionHandler(nil, customizedError)
      } else {
        MindvalleyImage.parseJSONWithCompletionHandler(data: data!, completionHandler: completionHandler)
      }
  ...
  func taskForGetImageWith(urlString: String, parameters: [String : AnyObject]?=nil, size:Int?=nil, completionHandler: @escaping CompletionHandler)->URLSessionDataTask? {
  ...
  class func errorForData(data: Data?, response: URLResponse?, error:  Error) -> Error {
    ...
    do {
      ...
      if let parsedResult = parsedResult as? [String : AnyObject], let errorCode = parsedResult[Constants.JSONResponseKeys.ErrorInfo.Code] as? String {
        let errorMessage = parsedResult[errorCode]
        let userInfo = [NSLocalizedDescriptionKey : errorMessage as Any]
        return NSError(domain: Constants.JSONResponseKeys.ErrorInfo.Domain, code:Int(errorCode)! , userInfo: userInfo) as Error
      }
  ...
  class func parseJSONWithCompletionHandler(data: Data, completionHandler: CompletionHandler) {
